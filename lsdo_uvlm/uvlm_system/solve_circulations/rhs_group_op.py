...
from lsdo_uvlm.uvlm_system.solve_circulations.kinematic_velocity_comp import KinematicVelocityComp
from lsdo_uvlm.uvlm_system.solve_circulations.assemble_aic import AssembleAic
from lsdo_uvlm.uvlm_system.solve_circulations.compute_normal_comp import ComputeNormal
from lsdo_uvlm.uvlm_system.solve_circulations.projection_comp import Projection
from lsdo_uvlm.uvlm_system.solve_circulations.utils.compute_sprs import compute_spars
from lsdo_uvlm.uvlm_system.solve_circulations.utils.custom_explicit_mat_sprsmat import Explicit
...
class RHS(Model):
    ...
    def define(self):
        ...
        wake_coords_names = [x + '_wake_coords_int' for x in surface_names]
        bd_coll_pts_shapes = [
        ...
        #     normals_normalized = self.declare_variable(surface_names[i]+'_bd_vtx_normals',
        #                                             val=np.einsum('ijk,l->ijkl',np.ones((num_nodes,bd_vortex_shapes[i][1]-1,bd_vortex_shapes[i][2]-1)),np.array([0,0,1])))
        kinematic_vel_names = [
        ...
        self.add(m, name='Projection_aic_M')
